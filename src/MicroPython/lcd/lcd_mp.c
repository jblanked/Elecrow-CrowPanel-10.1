...
{
    // args[0] is self, args[1] is x, args[2] is y, args[3] is c, args[4] is color
    uint16_t x = mp_obj_get_int(args[1]);
    uint16_t y = mp_obj_get_int(args[2]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(lcd_mp_draw_char_obj, 5, 5, lcd_mp_draw_char);

STATIC mp_obj_t lcd_mp_draw_text(size_t n_args, const mp_obj_t *args)
{
    // args[0] is self, args[1] is x, args[2] is y, args[3] is text, args[4] is color
    uint16_t x = mp_obj_get_int(args[1]);
    uint16_t y = mp_obj_get_int(args[2]);
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(lcd_mp_draw_text_obj, 5, 5, lcd_mp_draw_text);
